...
function find_record(){
   ...
   count=$(awk 'NR>1' $FILE | grep -icE "^$1:|:$1:|:$1\$")
...
function update_record(){
    #finding entries
   awk 'NR>1' $FILE | grep -iE "^$1:|:$1:|:$1\$" > temp_file
...
function sort_records(){
    while true; do
        ...
        case "$entry" in
            1) cat temp_file | sort -o temp_file
              echo;;
            2) cat temp_file | sort -t: -k2 -d -o temp_file
               echo;;
            3) cat temp_file | sort -t: -k3 -d -o temp_file
               echo;;
            4) cat temp_file | sort -t: -k7 -d -o temp_file
              echo;;
            *) echo "***Invalid entry.***"
               break;;
        esac
